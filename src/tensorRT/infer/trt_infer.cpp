...
struct Profiler : public nvinfer1::IProfiler
{
    ...
    void printLayerTimes()
    {
        ...
        {
            printf("%-80.80s %4.3fms\n", mProfile[i].first.c_str(), mProfile[i].second / TIMING_ITERATIONS);
			append_to_file("Time.log", iLogger::format("%-80.80s %4.3fms", mProfile[i].first.c_str(), mProfile[i].second / TIMING_ITERATIONS));
            totalTime += mProfile[i].second;
        }
        printf("Time over all layers: %4.3f\n", totalTime / TIMING_ITERATIONS);
		append_to_file("Time.log", iLogger::format("Time over all layers: %4.3f\n", totalTime / TIMING_ITERATIONS));
